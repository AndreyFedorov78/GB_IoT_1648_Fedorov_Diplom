#include "app_ModBus.h"
#include "queue.h"
#include "stdio.h"
#include "stdint.h"
#include "functions.h"
#include "string.h"
#include "stdlib.h"

extern osMessageQId modbusQueueHandle;
extern dev_settings system_settings;
extern osMessageQId MQTT_MessagesHandle;
extern char mlog_string[LOG_STRING_SIZE];
extern osTimerId modBusTimerHandle;

const uint16_t crctable[] = { 0x0000, 0xC0C1, 0xC181, 0x0140, 0xC301, 0x03C0,
		0x0280, 0xC241, 0xC601, 0x06C0, 0x0780, 0xC741, 0x0500, 0xC5C1, 0xC481,
		0x0440, 0xCC01, 0x0CC0, 0x0D80, 0xCD41, 0x0F00, 0xCFC1, 0xCE81, 0x0E40,
		0x0A00, 0xCAC1, 0xCB81, 0x0B40, 0xC901, 0x09C0, 0x0880, 0xC841, 0xD801,
		0x18C0, 0x1980, 0xD941, 0x1B00, 0xDBC1, 0xDA81, 0x1A40, 0x1E00, 0xDEC1,
		0xDF81, 0x1F40, 0xDD01, 0x1DC0, 0x1C80, 0xDC41, 0x1400, 0xD4C1, 0xD581,
		0x1540, 0xD701, 0x17C0, 0x1680, 0xD641, 0xD201, 0x12C0, 0x1380, 0xD341,
		0x1100, 0xD1C1, 0xD081, 0x1040, 0xF001, 0x30C0, 0x3180, 0xF141, 0x3300,
		0xF3C1, 0xF281, 0x3240, 0x3600, 0xF6C1, 0xF781, 0x3740, 0xF501, 0x35C0,
		0x3480, 0xF441, 0x3C00, 0xFCC1, 0xFD81, 0x3D40, 0xFF01, 0x3FC0, 0x3E80,
		0xFE41, 0xFA01, 0x3AC0, 0x3B80, 0xFB41, 0x3900, 0xF9C1, 0xF881, 0x3840,
		0x2800, 0xE8C1, 0xE981, 0x2940, 0xEB01, 0x2BC0, 0x2A80, 0xEA41, 0xEE01,
		0x2EC0, 0x2F80, 0xEF41, 0x2D00, 0xEDC1, 0xEC81, 0x2C40, 0xE401, 0x24C0,
		0x2580, 0xE541, 0x2700, 0xE7C1, 0xE681, 0x2640, 0x2200, 0xE2C1, 0xE381,
		0x2340, 0xE101, 0x21C0, 0x2080, 0xE041, 0xA001, 0x60C0, 0x6180, 0xA141,
		0x6300, 0xA3C1, 0xA281, 0x6240, 0x6600, 0xA6C1, 0xA781, 0x6740, 0xA501,
		0x65C0, 0x6480, 0xA441, 0x6C00, 0xACC1, 0xAD81, 0x6D40, 0xAF01, 0x6FC0,
		0x6E80, 0xAE41, 0xAA01, 0x6AC0, 0x6B80, 0xAB41, 0x6900, 0xA9C1, 0xA881,
		0x6840, 0x7800, 0xB8C1, 0xB981, 0x7940, 0xBB01, 0x7BC0, 0x7A80, 0xBA41,
		0xBE01, 0x7EC0, 0x7F80, 0xBF41, 0x7D00, 0xBDC1, 0xBC81, 0x7C40, 0xB401,
		0x74C0, 0x7580, 0xB541, 0x7700, 0xB7C1, 0xB681, 0x7640, 0x7200, 0xB2C1,
		0xB381, 0x7340, 0xB101, 0x71C0, 0x7080, 0xB041, 0x5000, 0x90C1, 0x9181,
		0x5140, 0x9301, 0x53C0, 0x5280, 0x9241, 0x9601, 0x56C0, 0x5780, 0x9741,
		0x5500, 0x95C1, 0x9481, 0x5440, 0x9C01, 0x5CC0, 0x5D80, 0x9D41, 0x5F00,
		0x9FC1, 0x9E81, 0x5E40, 0x5A00, 0x9AC1, 0x9B81, 0x5B40, 0x9901, 0x59C0,
		0x5880, 0x9841, 0x8801, 0x48C0, 0x4980, 0x8941, 0x4B00, 0x8BC1, 0x8A81,
		0x4A40, 0x4E00, 0x8EC1, 0x8F81, 0x4F40, 0x8D01, 0x4DC0, 0x4C80, 0x8C41,
		0x4400, 0x84C1, 0x8581, 0x4540, 0x8701, 0x47C0, 0x4680, 0x8641, 0x8201,
		0x42C0, 0x4380, 0x8341, 0x4100, 0x81C1, 0x8081, 0x4040 };

uint8_t mb_buf_tx[MB_BUF_TX_SIZE], mb_buf_rx[MB_BUF_RX_SIZE];

const uint8_t mbDataType[COMMANDS_QUANTITY] = {'C','I','R','R','C','R'};

char local_log[LOG_STRING_SIZE];
extern dev_settings system_settings;
char *command = system_settings.mb_program-1;

void ModBusCallback(void const *argument) {
	/* USER CODE BEGIN ModBusCallback */
	if (strlen(system_settings.mb_program) == 0){
		return;
	}
	if (*command == 0) command = system_settings.mb_program;
	else command++;
	uint8_t mb_command[8];
	for (int i = 0; i < 12; i += 2) {
		char hex_byte[3];
		hex_byte[0] = command[i];
		hex_byte[1] = command[i + 1];
		hex_byte[2] = '\0';
		mb_command[i / 2] = (uint8_t) strtol(hex_byte, NULL, 16);
	}
	xQueueSend(modbusQueueHandle, mb_command, 5);
	command += 12;

	/* USER CODE END ModBusCallback */
}

void modbus(void const *argument) {
	/* USER CODE BEGIN modbus */
	osTimerStart(modBusTimerHandle, 3000);
	/* Infinite loop */
	for (;;) {

		BaseType_t result = xQueueReceive(modbusQueueHandle, mb_buf_tx, 1000);

		if (pdPASS == result) {
			MX_USART1_UART_Init();
            // huart1.Instance->BRR = SystemCoreClock / system_settings.uart_speed;
			mb_buf_tx[6] = (uint8_t) GetCRC16(mb_buf_tx, 6);
			mb_buf_tx[7] = (uint8_t) (GetCRC16(mb_buf_tx, 6) >> 8);
			HAL_GPIO_WritePin(RS485_GPIO_Port, RS485_Pin, 1);
			HAL_UART_Transmit(&huart1, mb_buf_tx, 8, 100);
			HAL_GPIO_WritePin(RS485_GPIO_Port, RS485_Pin, 0);
			int l = modbusGetAnswer();

            osDelay(2);
			int x = sprintf(local_log, "modbus command __n__        ->  ");
			for (int i = 0; i < 6; i++) {
				x += sprintf(&local_log[x], "%02x ", mb_buf_tx[i]);
			}
			x += sprintf(&local_log[x], "__n__        <-  ");
			for (int i = 0; i < (l - 2); i++) {
				x += sprintf(&local_log[x], "%02x ", mb_buf_rx[i]);
			}


			osDelay(2);
				int rc= strlcpy(mlog_string, local_log, LOG_STRING_SIZE);
			if (checkCRC16(mb_buf_rx, l) && mb_buf_tx[1]<=COMMANDS_QUANTITY) {
				MQTT_Packet msg;
			    snprintf(msg.topic,50,"%s/%lu/%c-%ld",
                        system_settings.mqtt_topic,
			    		(uint32_t)system_settings.device_mac_address + mb_buf_rx[0],
						mbDataType[mb_buf_tx[1]-1],
						readHEX(&(mb_buf_tx[2]), 2));
			    sprintf(msg.message,"%ld", readHEX(&(mb_buf_rx[l-4]), 2));
			    xQueueSend(MQTT_MessagesHandle, &msg, 50);

			}

		}

	}
	/* USER CODE END modbus */
}

/*
 1 (0x01) — чтение значений из нескольких регистров флагов (Read Coil Status).
 2 (0x02) — чтение значений из нескольких дискретных входов (Read Discrete Inputs).
 3 (0x03) — чтение значений из нескольких регистров хранения (Read Holding Registers).
 4 (0x04) — чтение значений из нескольких регистров ввода (Read Input Registers).
 5 (0x05) — запись значения одного флага (Force Single Coil).
 6 (0x06) — запись значения в один регистр хранения (Preset Single Register).
 */

uint8_t modbusReadCoilStatus(uint8_t device, uint16_t address) {
	modbusSendCommand(device, READ_COIL_STATUS, address, 0x0008);
	int size = modbusGetAnswer();
	if (size && checkCRC16(mb_buf_rx, size)) {
		if (mb_buf_rx[0] != device)
			return 0;
		if (mb_buf_rx[1] != READ_COIL_STATUS)
			return 0;
		return mb_buf_rx[3];
	}
	return 0;
}

uint8_t modbusReadDiscreteInputs(uint8_t device, uint16_t address) {
	modbusSendCommand(device, READ_DISCRETE_INPUTS, address, 0x0008);
	int size = modbusGetAnswer();
	if (size && checkCRC16(mb_buf_rx, size)) {
		if (mb_buf_rx[0] != device)
			return 0;
		if (mb_buf_rx[1] != READ_DISCRETE_INPUTS)
			return 0;
		return mb_buf_rx[3];
	}
	return 0;
}

int modbusGetAnswer() {
	//uint8_t symbol;
	uint8_t size = MB_BUF_RX_SIZE;
	uint8_t *buff = mb_buf_rx;
	uint16_t  uartDelay =100;
	huart1.RxState = HAL_UART_STATE_READY;  // TODO: наверное стоит далать это как-то более обдумано напрмер сделать обработчик ошибок UART
	while (HAL_UART_Receive(&huart1, buff, 1, uartDelay) == HAL_OK && size) {
		//*buff = symbol;
		uartDelay=50;
		buff++;
		size--;
	}
	return MB_BUF_RX_SIZE - size;
}

int modbusSendCommand(uint8_t devise, uint8_t command, uint16_t address,
		uint16_t data) {
	mb_buf_tx[0] = devise;
	mb_buf_tx[1] = command;
	mb_buf_tx[2] = (uint8_t) (address >> 8);
	mb_buf_tx[3] = (uint8_t) address;
	mb_buf_tx[4] = (uint8_t) (data >> 8);
	mb_buf_tx[5] = (uint8_t) data;
	mb_buf_tx[6] = (uint8_t) GetCRC16(mb_buf_tx, 6);
	mb_buf_tx[7] = (uint8_t) (GetCRC16(mb_buf_tx, 6) >> 8);
	HAL_GPIO_WritePin(RS485_GPIO_Port, RS485_Pin, 1);
	HAL_UART_Transmit(&huart1, mb_buf_tx, 8, 100);
	HAL_GPIO_WritePin(RS485_GPIO_Port, RS485_Pin, 0);
	return 0;
}

uint16_t GetCRC16(uint8_t *buf, uint8_t len) {
	uint16_t crc;
	crc = 0xFFFF;
	while (len--) {
		crc = crctable[((crc) ^ ((uint8_t) *buf)) & 0xFF] ^ (crc >> 8);
		buf++;
	}
	return crc;
}

uint8_t checkCRC16(uint8_t *buf, uint8_t len) {
	if (len < 3) {
		return 0;
	}
	return !( GetCRC16(buf, len - 2)- buf[len - 2] - buf[len - 1]*256)   ;
}






uint32_t readHEX(uint8_t *buf, uint8_t size) {
    uint32_t result = 0;
    for (uint8_t i=0; i< size;i++) {
        result = *(buf++) + result * 256;
    }
    return result;
}
